# -*- coding: utf-8 -*-
"""
Created on Thu Dec  5 17:59:11 2019
...
known_sats = [9191, 10814, 15956, 16617, 30206, 33499, 74562, 79606, 87337, 87343, 74415]
...
def gather_data(file):
    ...
    astroy_sats = [68, 5855, 6916, 8333, 8740, 9140, 9191, 9356, 9594, 10814, 
                   ...
                   88854, 89581, 93428, 93836, 95609, 95991, 96484, 96564, 99277, 74415]
    
    for sat in known_sats:
        astroy_sats.remove(sat)
    
    for i in range(len(data)):
        row = data.iloc[i]
        if row["NORAD_CAT_ID"] != current_id and row["NORAD_CAT_ID"] not in astroy_sats:
...
important_data = []
for sat in data:
    important_data.append([sat['INCLINATION'], sat['SEMIMAJOR_AXIS'],sat['MEAN_MOTION'], sat['RA_OF_ASC_NODE']])

k_means = KMeans(n_clusters=100, init='k-means++', random_state=0, )
...
#%%
for sat in classes[57]:
    if sat not in known_sats:
        print(sat)
